...
func TestConfig_IsAthenticated(t *testing.T) {
	req, _ := http.NewRequest("GET", "/", nil)

	ctx := GetCtx(req)

	req = req.WithContext(ctx)
	auth := testApp.IsAthenticated(req)

	if auth {
		t.Error("returns true for athenticated, it expected false")
	}
	...
	if !auth {
		t.Error("returns false for athenticated, it expected true")
	}
}
